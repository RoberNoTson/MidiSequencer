...
// STATIC vars
snd_seq_t *MIDI_SEQ::seq=0;
snd_seq_addr_t *MIDI_SEQ::ports=0;
double MIDI_SEQ::song_length_seconds=0;
unsigned int MIDI_SEQ::event_num=0;
...
// constructor
MIDI_SEQ::MIDI_SEQ(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::MIDI_SEQ)
{
    ui->setupUi(this);
    ...
    init_seq();
    queue = snd_seq_alloc_named_queue(seq, "midi_seq");
...
//  SLOTS
void MIDI_SEQ::on_Open_button_clicked()
{
    ui->Play_button->setChecked(false);
...
void MIDI_SEQ::on_Play_button_toggled(bool checked)
{
    if (checked) {
...
void MIDI_SEQ::on_Panic_button_clicked()
{
  char buf[6];
  if (seq) {
    connect_port();
...
void MIDI_SEQ::send_SysEx(char * buf,int data_size) {
...
void MIDI_SEQ::init_seq() {
    if (!seq) {
        int err = snd_seq_open(&seq, "default", SND_SEQ_OPEN_OUTPUT, 0);
        check_snd("open sequencer", err);
        err = snd_seq_set_client_name(seq, "midi_seq");
...
void MIDI_SEQ::connect_port() {
    if (seq && strlen(port_name)) {
        ...
        snd_seq_port_info_set_name(pinfo, "midi_seq");
...
void MIDI_SEQ::getPorts(QString buf) {
...
void MIDI_SEQ::getRawDev(QString buf) {
	 ...
	 if (all_events[event_num].type==SND_SEQ_EVENT_CONTROLLER) {
	   if (all_events[event_num].data.d[1]==7) {		// Vol change
		...
		ui->MIDI_Volume_1->blockSignals(false);
	      ...
	      (ui->MIDI_Volume_1->value() ? (ui->MIDI_Expression_1->value() ? ui->MIDI_Expression_1->value() : 127 ) : 0) +
	      ui->MIDI_Volume_1->value()) / 3);
		break;
	      case 1:
		ui->MIDI_Volume_2->blockSignals(true);
		...
		ui->MIDI_Volume_2->blockSignals(false);
	      ...
	      (ui->MIDI_Volume_2->value() ? (ui->MIDI_Expression_2->value() ? ui->MIDI_Expression_2->value() : 127 ) : 0) +
	      ui->MIDI_Volume_2->value()) / 3);
		break;
	      case 2:
		ui->MIDI_Volume_3->blockSignals(true);
		...
		ui->MIDI_Volume_3->blockSignals(false);
	      ...
	      (ui->MIDI_Volume_3->value() ? (ui->MIDI_Expression_3->value() ? ui->MIDI_Expression_3->value() : 127 ) : 0) +
	      ui->MIDI_Volume_3->value()) / 3);
		break;
	      case 3:
		ui->MIDI_Volume_4->blockSignals(true);
		...
		ui->MIDI_Volume_4->blockSignals(false);
	      ...
	      (ui->MIDI_Volume_4->value() ? (ui->MIDI_Expression_4->value() ? ui->MIDI_Expression_4->value() : 127 ) : 0) +
	      ui->MIDI_Volume_4->value()) / 3);
		break;
	      case 4:
		ui->MIDI_Volume_5->blockSignals(true);
		...
		ui->MIDI_Volume_5->blockSignals(false);
	      ...
	      (ui->MIDI_Volume_5->value() ? (ui->MIDI_Expression_5->value() ? ui->MIDI_Expression_5->value() : 127 ) : 0) +
	      ui->MIDI_Volume_5->value()) / 3);
		break;
	      case 5:
		ui->MIDI_Volume_6->blockSignals(true);
		...
		ui->MIDI_Volume_6->blockSignals(false);
	      ...
	      (ui->MIDI_Volume_6->value() ? (ui->MIDI_Expression_6->value() ? ui->MIDI_Expression_6->value() : 127 ) : 0) +
	      ui->MIDI_Volume_6->value()) / 3);
		break;
	      case 6:
		ui->MIDI_Volume_7->blockSignals(true);
		...
		ui->MIDI_Volume_7->blockSignals(false);
	      ...
	      (ui->MIDI_Volume_7->value() ? (ui->MIDI_Expression_7->value() ? ui->MIDI_Expression_7->value() : 127 ) : 0) +
	      ui->MIDI_Volume_7->value()) / 3);
		break;
	      case 7:
		ui->MIDI_Volume_8->blockSignals(true);
		...
		ui->MIDI_Volume_8->blockSignals(false);
	      ...
	      (ui->MIDI_Volume_8->value() ? (ui->MIDI_Expression_8->value() ? ui->MIDI_Expression_8->value() : 127 ) : 0) +
	      ui->MIDI_Volume_8->value()) / 3);
		break;
	      case 8:
		ui->MIDI_Volume_9->blockSignals(true);
		...
		ui->MIDI_Volume_9->blockSignals(false);
	      ...
	      (ui->MIDI_Volume_9->value() ? (ui->MIDI_Expression_9->value() ? ui->MIDI_Expression_9->value() : 127 ) : 0) +
	      ui->MIDI_Volume_9->value()) / 3);
		break;
	      case 9:
		ui->MIDI_Volume_10->blockSignals(true);
		...
		ui->MIDI_Volume_10->blockSignals(false);
	      ...
	      (ui->MIDI_Volume_10->value() ? (ui->MIDI_Expression_10->value() ? ui->MIDI_Expression_10->value() : 127 ) : 0) +
	      ui->MIDI_Volume_10->value()) / 3);
		break;
	      case 10:
		ui->MIDI_Volume_11->blockSignals(true);
		...
		ui->MIDI_Volume_11->blockSignals(false);
	      ...
	      (ui->MIDI_Volume_11->value() ? (ui->MIDI_Expression_11->value() ? ui->MIDI_Expression_11->value() : 127 ) : 0) +
	      ui->MIDI_Volume_11->value()) / 3);
		break;
	      case 11:
		ui->MIDI_Volume_12->blockSignals(true);
		...
		ui->MIDI_Volume_12->blockSignals(false);
	      ...
	      (ui->MIDI_Volume_12->value() ? (ui->MIDI_Expression_12->value() ? ui->MIDI_Expression_12->value() : 127 ) : 0) +
	      ui->MIDI_Volume_12->value()) / 3);
		break;
	      case 12:
		ui->MIDI_Volume_13->blockSignals(true);
		...
		ui->MIDI_Volume_13->blockSignals(false);
	      ...
	      (ui->MIDI_Volume_13->value() ? (ui->MIDI_Expression_13->value() ? ui->MIDI_Expression_13->value() : 127 ) : 0) +
	      ui->MIDI_Volume_13->value()) / 3);
		break;
	      case 13:
		ui->MIDI_Volume_14->blockSignals(true);
		...
		ui->MIDI_Volume_14->blockSignals(false);
	      ...
	      (ui->MIDI_Volume_14->value() ? (ui->MIDI_Expression_14->value() ? ui->MIDI_Expression_14->value() : 127 ) : 0) +
	      ui->MIDI_Volume_14->value()) / 3);
		break;
	      case 14:
		ui->MIDI_Volume_15->blockSignals(true);
		...
		ui->MIDI_Volume_15->blockSignals(false);
	      ...
	      (ui->MIDI_Volume_15->value() ? (ui->MIDI_Expression_15->value() ? ui->MIDI_Expression_15->value() : 127 ) : 0) +
	      ui->MIDI_Volume_15->value()) / 3);
		break;
	      case 15:
		ui->MIDI_Volume_16->blockSignals(true);
		...
		ui->MIDI_Volume_16->blockSignals(false);
	      ...
	      (ui->MIDI_Volume_16->value() ? (ui->MIDI_Expression_16->value() ? ui->MIDI_Expression_16->value() : 127 ) : 0) +
	      ui->MIDI_Volume_16->value()) / 3);
		break;
	      default:
		break;
	    } // end SWITCH
...
//	      expr = ui->MIDI_Volume_1->value() ? (ui->MIDI_Expression_1->value() ? ui->MIDI_Expression_1->value() : 127 ) : 0;
	    ...
	    case 1:
	      ...
	      expr = ui->MIDI_Volume_2->value() ? (ui->MIDI_Expression_2->value() ? ui->MIDI_Expression_2->value() : 127 ) : 0;
	      ui->MIDI_VolDisp_2->setValue((all_events[event_num].data.d[2] + expr + ui->MIDI_Volume_2->value()) / 3);
	      break;
	    case 2:
	      if (ui->MIDI_VolDisp_3->value()) ui->MIDI_VolDisp_3->setValue(0);
	      expr = ui->MIDI_Volume_3->value() ? (ui->MIDI_Expression_3->value() ? ui->MIDI_Expression_3->value() : 127 ) : 0;
	      ui->MIDI_VolDisp_3->setValue((all_events[event_num].data.d[2] + expr + ui->MIDI_Volume_3->value()) / 3);
	      break;
	    case 3:
	      if (ui->MIDI_VolDisp_4->value()) ui->MIDI_VolDisp_4->setValue(0);
	      expr = ui->MIDI_Volume_4->value() ? (ui->MIDI_Expression_4->value() ? ui->MIDI_Expression_4->value() : 127 ) : 0;
	      ui->MIDI_VolDisp_4->setValue((all_events[event_num].data.d[2] + expr + ui->MIDI_Volume_4->value()) / 3);
	      break;
	    case 4:
	      if (ui->MIDI_VolDisp_5->value()) ui->MIDI_VolDisp_5->setValue(0);
	      expr = ui->MIDI_Volume_5->value() ? (ui->MIDI_Expression_5->value() ? ui->MIDI_Expression_5->value() : 127 ) : 0;
	      ui->MIDI_VolDisp_5->setValue((all_events[event_num].data.d[2] + expr + ui->MIDI_Volume_5->value()) / 3);
	      break;
	    case 5:
	      if (ui->MIDI_VolDisp_6->value()) ui->MIDI_VolDisp_6->setValue(0);
	      expr = ui->MIDI_Volume_6->value() ? (ui->MIDI_Expression_6->value() ? ui->MIDI_Expression_6->value() : 127 ) : 0;
	      ui->MIDI_VolDisp_6->setValue((all_events[event_num].data.d[2] + expr + ui->MIDI_Volume_6->value()) / 3);
	      break;
	    case 6:
	      if (ui->MIDI_VolDisp_7->value()) ui->MIDI_VolDisp_7->setValue(0);
	      expr = ui->MIDI_Volume_7->value() ? (ui->MIDI_Expression_7->value() ? ui->MIDI_Expression_7->value() : 127 ) : 0;
	      ui->MIDI_VolDisp_7->setValue((all_events[event_num].data.d[2] + expr + ui->MIDI_Volume_7->value()) / 3);
	      break;
	    case 7:
	      if (ui->MIDI_VolDisp_8->value()) ui->MIDI_VolDisp_8->setValue(0);
	      expr = ui->MIDI_Volume_8->value() ? (ui->MIDI_Expression_8->value() ? ui->MIDI_Expression_8->value() : 127 ) : 0;
	      ui->MIDI_VolDisp_8->setValue((all_events[event_num].data.d[2] + expr + ui->MIDI_Volume_8->value()) / 3);
	      break;
	    case 8:
	      if (ui->MIDI_VolDisp_9->value()) ui->MIDI_VolDisp_9->setValue(0);
	      expr = ui->MIDI_Volume_9->value() ? (ui->MIDI_Expression_9->value() ? ui->MIDI_Expression_9->value() : 127 ) : 0;
	      ui->MIDI_VolDisp_9->setValue((all_events[event_num].data.d[2] + expr + ui->MIDI_Volume_9->value()) / 3);
	      break;
	    case 9:
//	      if (ui->MIDI_VolDisp_10->value()) ui->MIDI_VolDisp_10->setValue(0);
	      expr = ui->MIDI_Volume_10->value() ? (ui->MIDI_Expression_10->value() ? ui->MIDI_Expression_10->value() : 127 ) : 0;
	      ui->MIDI_VolDisp_10->setValue((all_events[event_num].data.d[2] + expr + ui->MIDI_Volume_10->value()) / 3);
	      break;
	    case 10:
	      if (ui->MIDI_VolDisp_11->value()) ui->MIDI_VolDisp_11->setValue(0);
	      expr = ui->MIDI_Volume_11->value() ? (ui->MIDI_Expression_11->value() ? ui->MIDI_Expression_11->value() : 127 ) : 0;
	      ui->MIDI_VolDisp_11->setValue((all_events[event_num].data.d[2] + expr + ui->MIDI_Volume_11->value()) / 3);
	      break;
	    case 11:
	      if (ui->MIDI_VolDisp_12->value()) ui->MIDI_VolDisp_12->setValue(0);
	      expr = ui->MIDI_Volume_12->value() ? (ui->MIDI_Expression_12->value() ? ui->MIDI_Expression_12->value() : 127 ) : 0;
	      ui->MIDI_VolDisp_12->setValue((all_events[event_num].data.d[2] + expr + ui->MIDI_Volume_12->value()) / 3);
	      break;
	    case 12:
	      if (ui->MIDI_VolDisp_13->value()) ui->MIDI_VolDisp_13->setValue(0);
	      expr = ui->MIDI_Volume_13->value() ? (ui->MIDI_Expression_13->value() ? ui->MIDI_Expression_13->value() : 127 ) : 0;
	      ui->MIDI_VolDisp_13->setValue((all_events[event_num].data.d[2] + expr + ui->MIDI_Volume_13->value()) / 3);
	      break;
	    case 13:
	      if (ui->MIDI_VolDisp_14->value()) ui->MIDI_VolDisp_14->setValue(0);
	      expr = ui->MIDI_Volume_14->value() ? (ui->MIDI_Expression_14->value() ? ui->MIDI_Expression_14->value() : 127 ) : 0;
	      ui->MIDI_VolDisp_14->setValue((all_events[event_num].data.d[2] + expr + ui->MIDI_Volume_14->value()) / 3);
	      break;
	    case 14:
	      if (ui->MIDI_VolDisp_15->value()) ui->MIDI_VolDisp_15->setValue(0);
	      expr = ui->MIDI_Volume_15->value() ? (ui->MIDI_Expression_15->value() ? ui->MIDI_Expression_15->value() : 127 ) : 0;
	      ui->MIDI_VolDisp_15->setValue((all_events[event_num].data.d[2] + expr + ui->MIDI_Volume_15->value()) / 3);
	      break;
	    case 15:
	      if (ui->MIDI_VolDisp_16->value()) ui->MIDI_VolDisp_16->setValue(0);
	      expr = ui->MIDI_Volume_16->value() ? (ui->MIDI_Expression_16->value() ? ui->MIDI_Expression_16->value() : 127 ) : 0;
	      ui->MIDI_VolDisp_16->setValue((all_events[event_num].data.d[2] + expr + ui->MIDI_Volume_16->value()) / 3);
	      break;
	  } // end switch NOTEON
	} // end IF NOTEON
	// scan for Note On, set volume disp
...
void MIDI_SEQ::startPlayer(int startTick) {
    if (pid>0) 
      return;
    pid=fork();
    if (!pid) {
...
void MIDI_SEQ::on_MIDI_Volume_Master_valueChanged(int val) {
    char buf[8];
...
void MIDI_SEQ::on_MIDI_Exit_button_clicked() {
    this->close();
}

void MIDI_SEQ::on_MIDI_GMGS_button_toggled(bool checked) {
  if (checked) {
